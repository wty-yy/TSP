...
bool map_flag = 0;//如果map_flag=0就以邻接矩阵输入，否则是输入城市坐标
int Map[N][N];//用于存储邻接矩阵
double getdis(int i, int j) {
    if (map_flag) return sqrt((x[i]-x[j])*(x[i]-x[j]) + (y[i]-y[j])*(y[i]-y[j]));
    return Map[i][j];
}
//构图----------------------------------------------------------
...
void makemap() {
    ...
    for (int i = 1; i <= n; i++) if (!vis[i] && nt[i]) {
        vis[i] = 1;
...
void single() {
    for (int i = 1; i <= n; i++) if (!nt[i]) {
        mn = 1e100;
        memset(vis, 0, sizeof(vis));
        vis[i] = 1;
        for (int j = 1; j <= n; j++) if (!vis[j]) dfs2(j, i);
...
void greed(int u, int st, int use) {
    ...
    if (getdis(u, st) < mn && use > 1) {//这里允许由两个点构成一个强连通分量
        nt[u] = st;
        return;
    }
    nt[u] = v;
...
int main() {
    freopen("150points_matrix.in", "r", stdin);
